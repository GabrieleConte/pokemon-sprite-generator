...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, text_emb: torch.Tensor) -> torch.Tensor:
        ...
        residual = x_flat
        try:
            x_norm = self.norm1(x_flat.permute(0, 2, 1)).permute(0, 2, 1)                                 
        ...
        residual = x_flat
        try:
            x_norm = self.norm2(x_flat.permute(0, 2, 1)).permute(0, 2, 1)                                 
...
class UNet(nn.Module):
    ...
    def forward(self, noisy_latent: torch.Tensor, timesteps: torch.Tensor, 
        ...
        text_pooled = self.text_pool(text_emb.transpose(1, 2)).squeeze(-1)                 
...
def test_unet():
    ...
    batch_size = 4
    noisy_latent = torch.randn(batch_size, 8, 27, 27).to(device)                     
